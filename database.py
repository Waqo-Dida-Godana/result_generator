...
class Database:
    ...
    def _migrate_schema(self):
        ...
        """)
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_promotion_history_student_year ON promotion_history(student_id, academic_year)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_promotion_history_year ON promotion_history(academic_year)"
        )
        ...
        """)
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_promotion_audit_batch ON promotion_audit_log(promotion_batch_id)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_promotion_audit_time ON promotion_audit_log(performed_at)"
        )
    ...
    def _ensure_stream_scoped_teacher_assignments(self, conn, cursor):
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='teacher_assignments'"
        )
        if not cursor.fetchone():
            return
        ...
        if "stream_name" in columns and expected_unique in unique_indexes:
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_teacher_assignments_class_stream ON teacher_assignments(class_name, stream_name)"
            )
            cursor.execute(
        ...
        stream_select = (
            "COALESCE(stream_name, '')" if "stream_name" in columns else "''"
        )
        ...
        cursor.execute(
            "ALTER TABLE teacher_assignments_new RENAME TO teacher_assignments"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_teacher_assignments_class_stream ON teacher_assignments(class_name, stream_name)"
        )
    ...
    def _ensure_class_scoped_student_admission_numbers(self, conn, cursor):
        ...
        cursor.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_students_class_admission_no ON students(class, admission_no)"
        )
        cursor.execute("PRAGMA foreign_keys = ON")
    ...
    def update_teacher(
        ...
        try:
            ...
            conn.close()
            return (
                (True, "Teacher updated successfully")
                if success
                else (False, "Teacher not found")
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def delete_user(self, user_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM teacher_assignments WHERE teacher_id = ?", (user_id,)
            )

            # Delete the user
            cursor.execute(
                "DELETE FROM users WHERE id = ? AND role != ?", (user_id, "admin")
            )
    ...
    def assign_subject_teacher(
        self, teacher_id: str, class_name: str, subject: str, stream_name: str = ""
    ) -> bool:
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
    ...
    def assign_class_teacher(
        self, teacher_id: str, class_name: str, stream_name: str = ""
    ) -> bool:
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
    ...
    # ── Class Management ─────────────────────────────────────────────────
    def add_class(
        self, name: str, level: str, stream: str = None, abbreviation: str = ""
    ) -> Tuple[bool, str]:
        """Add a new class"""
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def update_class(
        ...
        try:
            ...
            if old_name != name:
                cursor.execute(
                    "UPDATE students SET class = ? WHERE class = ?", (name, old_name)
                )
                cursor.execute(
                    "UPDATE teacher_assignments SET class_name = ? WHERE class_name = ?",
                    (name, old_name),
                )
    ...
    def delete_class(self, class_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM teacher_assignments WHERE class_name = ?", (class_name,)
            )
    ...
    def get_streams_for_class(self, class_id: str) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM streams WHERE class_id = ? ORDER BY name", (class_id,)
        )
        rows = cursor.fetchall()
        conn.close()
        return [dict(r) for r in rows]
    ...
    def update_stream(
        self, stream_id: str, name: str, class_id: str
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            conn.close()
            return (
                (True, "Stream updated successfully")
                if success
                else (False, "Stream not found")
            )
        except Exception as e:
            conn.close()
            return False, str(e)
        ...
        try:
            ...
            cursor.execute(
                ...
                (
                    ...
                    subject_code,
                    subject_code or abbreviation.strip(),
                    datetime.now().isoformat(),
                ),
            )
            conn.commit()
    ...
    def update_subject(
        ...
        try:
            cursor.execute(
                "SELECT name FROM custom_subjects WHERE id = ?", (subject_id,)
            )
            row = cursor.fetchone()
            if not row:
                conn.close()
            ...
            if old_name != name:
                cursor.execute(
                    "UPDATE marks SET subject = ? WHERE subject = ?", (name, old_name)
                )
                cursor.execute(
                    "UPDATE teacher_assignments SET subject = ? WHERE subject = ?",
                    (name, old_name),
                )
            success = cursor.rowcount > 0 or old_name != name
            conn.commit()
            conn.close()
            return (
                (True, "Subject updated successfully")
                if success
                else (False, "Subject not found")
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def get_subjects_by_level(self, level: str = None) -> List[Dict]:
        ...
        if level:
            cursor.execute(
                "SELECT * FROM custom_subjects WHERE level = ? ORDER BY category, code, name",
                (level,),
            )
        else:
            cursor.execute(
                "SELECT * FROM custom_subjects ORDER BY level, category, code, name"
            )
        rows = cursor.fetchall()
        conn.close()
        return [dict(r) for r in rows]
    ...
    def get_subject_by_name(self, name: str, level: str = None) -> Optional[Dict]:
        ...
        if level:
            cursor.execute(
                "SELECT * FROM custom_subjects WHERE name = ? AND level = ?",
                (name, level),
            )
        else:
            cursor.execute(
                "SELECT * FROM custom_subjects WHERE name = ? ORDER BY level LIMIT 1",
                (name,),
            )
        row = cursor.fetchone()
        conn.close()
    ...
    def delete_subject(self, subject_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM teacher_assignments WHERE subject = ?", (subject_name,)
            )
    ...
    def update_grading_scale(
        ...
        try:
            ...
            conn.close()
            return (
                (True, "Grading scale updated successfully")
                if success
                else (False, "Grading scale not found")
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def get_students_by_class(self, class_name: str) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM students WHERE class = ? ORDER BY name", (class_name,)
        )
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]

    def get_students_by_class_and_stream(
        self, class_name: str, stream: str = ""
    ) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        if stream:
    ...
    def get_student_by_admission_no(
        self, admission_no: str, class_name: str = None
    ) -> Optional[Dict]:
        ...
        if class_name:
            ...
            cursor.execute(
                "SELECT * FROM students WHERE admission_no = ? ORDER BY updated_at DESC, created_at DESC LIMIT 1",
                (admission_no,),
            )
        row = cursor.fetchone()
        conn.close()
    ...
    def get_next_class_admission_no(
        self, class_name: str, exclude_student_id: str = ""
    ) -> str:
        conn = self.get_connection()
        cursor = conn.cursor()
        if exclude_student_id:
            ...
            cursor.execute(
                "SELECT admission_no FROM students WHERE class = ?", (class_name,)
            )
        rows = cursor.fetchall()
        conn.close()
    ...
    def delete_class_by_name(self, class_name: str) -> Tuple[bool, str]:
        ...
        try:
            # Get class ID first
            cursor.execute(
                "SELECT id FROM school_classes WHERE name = ?", (class_name,)
            )
            row = cursor.fetchone()
            if not row:
                conn.close()
            ...
            return (
                affected > 0,
                f"Deleted class and {len(student_ids)} students with their marks",
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def delete_all_students_in_class(self, class_name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                affected > 0,
                f"Deleted {affected} students and their marks from {class_name}",
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def delete_all_students(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                True,
                f"Deleted {students_deleted} students and {marks_deleted} marks.",
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def delete_all_subjects(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                True,
                f"Deleted {subjects_deleted} subjects, {marks_deleted} marks, and related assignments",
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def delete_all_classes(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            cursor.execute(
                "DELETE FROM teacher_assignments WHERE class_name IS NOT NULL"
            )
            ...
            return (
                True,
                f"Deleted {classes_deleted} classes, {students_deleted} students, {marks_deleted} marks, and related assignments",
            )
        except Exception as e:
            conn.close()
            return False, str(e)
    ...
    def get_setting(self, key: str, default: str = "") -> str:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT value FROM app_settings WHERE key = ?", (str(key).strip(),)
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def get_statistics(
        ...
        if marks_by_student:
            all_averages = [
                sum(m["marks"]) / len(m["marks"]) if m["marks"] else 0
                for m in marks_by_student.values()
            ]
            avg_score = (
                round(sum(all_averages) / len(all_averages), 1) if all_averages else 0
            )
            top_student = max(
                marks_by_student.values(),
                key=lambda x: sum(x["marks"]) if x["marks"] else 0,
            )
    ...
    def calculate_results(
        ...
        if class_filter == "All":
            ...
            cursor.execute(
                "SELECT * FROM students WHERE class = ? ORDER BY name", (class_filter,)
            )
    ...
    def get_all_classes_exam_history(self, academic_year: str = None) -> List[Dict]:
        ...
        for cls in classes:
            ...
            # Get student count
            cursor.execute(
                "SELECT COUNT(*) as count FROM students WHERE class = ?", (class_name,)
            )
            ...
            if exams:
                ...
                avg_score = (
                    round(avg_row["avg"], 1) if avg_row and avg_row["avg"] else None
                )
    ...
    def get_promotion_setting(self, key: str, default: str = "") -> str:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT value FROM promotion_settings WHERE key = ?", (str(key).strip(),)
        )
        row = cursor.fetchone()
        conn.close()
        return str(row["value"]) if row else default
    ...
    def _get_students_with_promotion_averages(
        self, class_name: str
    ...
    def get_students_eligible_for_promotion(
        self, class_name: str, min_average: float = 50.0
    ) -> List[Dict]:
        ...
        eligible = [
            student
            for student in students
            if student.get("marks_count", 0) > 0
            and student.get("average_marks") is not None
            and float(student["average_marks"]) >= float(min_average)
        ]
        return sorted(
            eligible,
            key=lambda item: (
                -float(item.get("average_marks") or 0),
                item.get("name", ""),
            ),
        )

    def get_students_repeating(
        self, class_name: str, min_average: float = 50.0
    ) -> List[Dict]:
    ...
    def get_students_without_promotion_data(self, class_name: str) -> List[Dict]:
        ...
        _, students = self._get_students_with_promotion_averages(class_name)
        no_data = [
            student for student in students if int(student.get("marks_count") or 0) == 0
        ]
    ...
    def _resolve_promotion_actor(
        self, cursor, performed_by: str = None
    ) -> Optional[str]:
    ...
    def _insert_promotion_audit_entry(
        self,
        cursor,
        batch_id: str,
        action: str,
    ...
    def _write_promotion_audit_entry(
        self,
        batch_id: str,
        action: str,
        details: str,
        ...
        try:
            ...
            self._insert_promotion_audit_entry(
                cursor, batch_id, action, details, actor_id, performed_at
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def batch_promote_students(
        self, promotions: List[Dict], academic_year: str, performed_by: str = None
    ) -> Tuple[bool, str, Dict]:
        ...
        try:
            actor_id = self._resolve_promotion_actor(cursor, performed_by)
            student_ids = [
                str(promo.get("student_id", "")).strip() for promo in promotions
            ]
            duplicate_student_ids = sorted(
                {sid for sid in student_ids if sid and student_ids.count(sid) > 1}
            )

            if duplicate_student_ids:
                results["failed"] = len(duplicate_student_ids)
                results["errors"] = [
                    f"Duplicate promotion entries for student {sid}"
                    for sid in duplicate_student_ids
                ]
                ...
                return (
                    False,
                    "Batch promotion rejected: duplicate students in request",
                    results,
                )
            ...
            for promo in promotions:
                ...
                status = (
                    str(promo.get("status", "promoted")).strip().lower() or "promoted"
                )
                ...
                if status not in ("promoted", "repeating"):
                    results["errors"].append(
                        f'Unsupported promotion status "{status}" for student {student_id}'
                    )
                    continue
                ...
                if student_id in already_processed:
                    results["errors"].append(
                        f"{student['name']} already has a promotion decision for {academic_year}"
                    )
                    continue
                ...
                if not to_class:
                    results["errors"].append(
                        f"Promotion target missing for {student['name']}"
                    )
                    continue
                if to_class != from_class and to_class not in valid_classes:
                    results["errors"].append(
                        f"Target class {to_class} does not exist for {student['name']}"
                    )
                    continue
    ...
    def get_promotion_history(
        self, student_id: str = None, academic_year: str = None, class_name: str = None
    ) -> List[Dict]:
    ...
    def get_promotion_audit_log(
        self, batch_id: str = None, limit: int = 100
    ) -> List[Dict]:
    ...
    def get_promotion_statistics(self, academic_year: str = None) -> Dict:
        ...
        return (
            dict(row)
            if row
            else {"total_promotions": 0, "promoted_count": 0, "repeating_count": 0}
        )

    def get_class_student_summary(
        self, class_name: str, stream_name: str = "", recent_term: str = None
    ) -> List[Dict]:
    ...
    def get_teacher_stats(self) -> Dict:
        """Get teacher-related statistics for dashboard."""
        conn = self.get_connection()
        cursor = conn.cursor()

        # Total teachers
        cursor.execute(
            "SELECT COUNT(*) as count FROM users WHERE role IN ('teacher', 'class_teacher')"
        )
        total_teachers = cursor.fetchone()["count"]

        # Subject teachers (non class-teacher assignments)
        cursor.execute("""
            SELECT COUNT(DISTINCT teacher_id) as count
            FROM teacher_assignments
            WHERE is_class_teacher = 0 AND subject IS NOT NULL
        """)
        subject_teachers = cursor.fetchone()["count"]
        ...
        """)
        class_teachers = cursor.fetchone()["count"]
    ...
    def get_class_stats(self) -> List[Dict]:
        """Get class-level statistics including student counts."""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            ...
            LEFT JOIN students s ON sc.name = s.class
            GROUP BY sc.id, sc.name, sc.level, sc.abbreviation
            ORDER BY sc.name
        """)
    ...
    def get_stream_stats(self) -> Dict:
        ...
        # Streams per class
        cursor.execute("""
            SELECT sc.name as class_name, COUNT(st.id) as stream_count
            FROM school_classes sc
            LEFT JOIN streams st ON sc.id = st.class_id
            GROUP BY sc.id, sc.name
            ORDER BY sc.name
        """)
        streams_by_class = {
            row["class_name"]: row["stream_count"] for row in cursor.fetchall()
        }
    ...
    def get_student_demographics(self) -> Dict:
        ...
        cursor.execute("""
            ...
            GROUP BY gender
        """)
        gender_data = {row["gender"]: row["count"] for row in cursor.fetchall()}

        # Stream distribution (students per stream)
        cursor.execute("""
            ...
            ORDER BY stream
        """)
        stream_distribution = {row["stream"]: row["count"] for row in cursor.fetchall()}
        ...
        cursor.execute("""
            ...
            ORDER BY class
        """)
        class_distribution = {row["class"]: row["count"] for row in cursor.fetchall()}
        ...
        return {
            ...
            "male": male,
            "female": female,
            "male_pct": round((male / total * 100) if total > 0 else 0, 1),
            "female_pct": round((female / total * 100) if total > 0 else 0, 1),
    ...
    def get_grade_distribution(
        ...
        """Get grade distribution across all students for a specific exam session."""
        conn = self.get_connection()
        cursor = conn.cursor()

        # Get all marks with student averages
        cursor.execute(
            """
            SELECT
                s.id,
        ...
        # Count grades
        distribution = {"EE": 0, "ME": 0, "AE": 0, "BE": 0, "IE": 0, "No Score": 0}
        for row in rows:
            avg = row["avg_marks"]
            if avg is None:
                distribution["No Score"] += 1
            elif avg >= 80:
                distribution["EE"] += 1
            elif avg >= 70:
                distribution["ME"] += 1
    ...
    def get_performance_stats(
        ...
        averages = [
            float(row["avg_marks"]) for row in rows if row["avg_marks"] is not None
        ]
        ...
        median_val = statistics.median(averages)
        min_val = min(averages)
        max_val = max(averages)
        std_dev = statistics.stdev(averages) if len(averages) > 1 else 0.0
        pass_count = sum(1 for avg in averages if avg >= 50)
        total = len(averages)
    ...
    def get_quick_navigation_stats(self) -> Dict:
        """Get quick stats for dashboard navigation cards."""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        # Streams count
        cursor.execute("SELECT COUNT(*) as count FROM streams")
        stats["streams"] = cursor.fetchone()["count"]
        ...
        # Subjects count
        cursor.execute("SELECT COUNT(*) as count FROM custom_subjects")
        stats["subjects"] = cursor.fetchone()["count"]

        # Recent exam sessions
        cursor.execute("""
            SELECT COUNT(DISTINCT term || '-' || exam_type) as count
            FROM marks
        """)
        stats["exam_sessions"] = cursor.fetchone()["count"]
