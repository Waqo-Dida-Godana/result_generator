...
class Database:
    ...
    def add_student(self, name: str, class_name: str, gender: str, admission_no: str) -> Dict:
        conn = self.get_connection()
        cursor = conn.cursor()
        student_id = str(uuid.uuid4())
        now = datetime.now().isoformat()
        cursor.execute(
            'INSERT INTO students (id, name, class, gender, admission_no, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
            (student_id, name, class_name, gender, admission_no, now, now)
        )
        conn.commit()
        conn.close()
        return {'id': student_id, 'name': name, 'class': class_name, 'gender': gender, 'admission_no': admission_no}
    
    def update_student(self, student_id: str, name: str, class_name: str, gender: str, admission_no: str) -> bool:
        conn = self.get_connection()
        cursor = conn.cursor()
        now = datetime.now().isoformat()
        cursor.execute(
